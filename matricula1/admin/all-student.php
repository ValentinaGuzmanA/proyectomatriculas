...
<table class="table  table-striped table-hover table-bordered" id="data">
  ...
  <tbody>
    <?php 
      $query=mysqli_query($db,'SELECT * FROM `student_info`');
      $i=1;
      ...
      <tr>
        ...
        echo '<td>'.$i.'</td>
          ...
          <td>'.ucwords($result['RegistroC']).'</td>
          <td>'.$result['adress'].'</td>
          <td>'.ucwords($result['FechaNacimiento']).'</td>
         
            <a class="btn btn-xs btn-warning" href="index.php?page=editstudent&id='.base64_encode($result['id']).'&adress='.base64_encode($result['adress']).'">
              <i class="fa fa-edit"></i></a>

             &nbsp; <a class="btn btn-xs btn-danger" onclick="javascript:confirmationDelete($(this));return false;" href="index.php?page=delete&id='.base64_encode($result['id']).'&adress='.base64_encode($result['adress']).'">
